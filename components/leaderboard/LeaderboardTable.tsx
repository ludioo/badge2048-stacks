...
export function LeaderboardTable({ entries, total, currentAddress }: LeaderboardTableProps) {
  if (entries.length === 0) {
    return (
    ...
    <div className="overflow-hidden rounded-xl border border-[#FD9E7F] bg-white shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[260px] text-left text-sm">
          ...
          <tbody>
            {entries.map((e) => {
              const isCurrentUser = currentAddress && e.address.toLowerCase() === currentAddress.toLowerCase()
              return (
                <tr
                  key={e.address}
                  className={`border-b border-[#FD9E7F]/20 last:border-b-0 hover:bg-[#FD9E7F]/5 ${
                    isCurrentUser
                      ? 'bg-[#FD9E7F]/15 border-l-4 border-l-[#F4622F] font-semibold'
                      : ''
                  }`}
                >
                  <td className={`px-3 sm:px-4 py-2.5 sm:py-3 font-medium ${isCurrentUser ? 'text-[#E8552A]' : 'text-[#F4622F]'}`}>
                    #{e.rank}
                  </td>
                  <td className={`px-3 sm:px-4 py-2.5 sm:py-3 font-mono ${isCurrentUser ? 'text-[#E8552A] font-semibold' : 'text-[#4B5563]'}`} title={e.address}>
                    {shortenAddress(e.address)}
                  </td>
                  <td className={`px-3 sm:px-4 py-2.5 sm:py-3 text-right font-semibold ${isCurrentUser ? 'text-[#E8552A]' : 'text-[#E8552A]'}`}>
